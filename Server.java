...
public class Server
{
	...
	boolean debugArg = false; // Set the debugArg to false by default
	if (args.length == 2) {
		if (args[1] == "debug") {
			debugArg = true; // If theres a second arugment and its "debug" then set debugArg to true
		}
		else {
			System.out.println("Usage: java Server port# [debug]");
			System.out.println("Second argument must either be \"debug\" or nothing");
			return;
		}
	}
	try {
	    Server s = new Server (Integer.parseInt(args[0]), debugArg);
	}
